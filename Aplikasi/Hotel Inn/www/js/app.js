...
var Application = {
    ...
    initShowDetailMhs : function (nim) {
        ...
        if(dataObject.nim == nim && tbdy.children().length == 0) {
            appendDetail = '<td><td>'+dataObject.nim+'</td></td>'+dataObject.nama+'</td><td>'+
            dataObject.jurusan+'</td><td>'+dataObject.fakultas+'</td><td>'+dataObject.fakultas+
            '</td><td>'+dataObject.alamat+'</td><td>'+dataObject.noHP+'</td></tr>';
        };
